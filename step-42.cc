...
// z_max_domain is the z value of the surface of the work piece
                                ...
                                const unsigned int component) const
    {
      ...
      if (use_read_obstacle)
        ...
        {
          //sphere:
          //      return z_max_domain - 0.01;// -std::sqrt(
          // 0.36 - (p(0) - 0.5) * (p(0) - 0.5)
          // - (p(1) - 0.5) * (p(1) - 0.5)) + z_max_domain + 0.59;
          //punch:
          double eps = 1e-12;
          if (p(0) >=0.25-eps && p(0) <= 0.75+eps
              && p(1) >= 0.25-eps && p(1) <= 0.75+eps)
            return z_max_domain - 0.001;
          else
            return 100000.0;
        }
    }
...
// starting value we solve an elastic problem in very first step (j=1).
  ...
  void PlasticityContactProblem<dim>::solve_newton()
  {
    ...
    pcout << " " <<std::endl;
    pcout << "      ||u||_L2 = " << old_solution.l2_norm() <<std::endl;
  }
...
//  Vector<float> estimated_error_per_cell_lambda(triangulation.n_active_cells());
//  KellyErrorEstimator < dim
//            > ::estimate(dof_handler, QGauss < dim - 1 > (fe.degree + 2),
//          typename FunctionMap<dim>::type(), lambda,
//          estimated_error_per_cell_lambda);

//  estimated_error_per_cell += estimated_error_per_cell_lambda;

//  parallel::distributed::GridRefinement::refine_and_coarsen_fixed_number(
//      triangulation, estimated_error_per_cell, to_refine_factor, 0.0);
        parallel::distributed::GridRefinement::refine_and_coarsen_fixed_number(
...
// Line plot from center point at contact boundary to point (0,0).
// Use cat LineInBody-0000.dat LineInBody-0001.dat > LineInBody.dat to merge
// the already sorted files.
//  {
//    std::string filename = ("LineInBody-"
...
//      Point < dim > p1(0.5 / 100.0 * p, 0.5 / 100.0 * p, p / 100.0);
//      point_found = true;
//      try {
//        solution_function.vector_value(p1, solution_p1);
//      } catch (const typename Functions::FEFieldFunction<dim,
//          DoFHandler<dim>, TrilinosWrappers::MPI::Vector>::ExcPointNotAvailableHere &) {
//        point_found = false;
//      }
//
//      if (point_found == true) {
//        solution_function.vector_gradient(p1, solution_gradient_p1);
//
//        // Calculating strains tensor
//        SymmetricTensor < 2, dim > strain_tensor;
//        for (unsigned int i = 0; i < dim; i++) {
...
//        stress_tensor = stress_strain_tensor * strain_tensor;
//
//        // output order: position along line + displacement + stress tensor
//        // position ux  uy  uz  sxx  syy  szz  sxy  sxz  syz
//        file << sqrt(3.0 / 2.0) * p / 100.0 << " " << solution_p1(0)
//            << " " << solution_p1(1) << " " << solution_p1(2) << " "
//            << stress_tensor[0][0] << " " << stress_tensor[1][1]
//            << " " << stress_tensor[2][2] << " "
//            << stress_tensor[0][1] << " " << stress_tensor[1][2]
//            << " " << stress_tensor[1][2] << std::endl;
//      }
//    }
//    file.close();
//  }

// Line plot over the contact boundary from (0.5,0.0) to (0.5,1.0)
//  {
//    // Calculation of the contact forces
//    TrilinosWrappers::MPI::Vector lambda(solution);
//    TrilinosWrappers::MPI::Vector distributed_lambda(system_rhs_newton);
//    distributed_lambda.ratio(resid_vector, diag_mass_matrix_vector);
//    lambda = distributed_lambda;
//
//    // To get access to the contact forces in lambda.
//    Functions::FEFieldFunction<dim, DoFHandler<dim>,
//        TrilinosWrappers::MPI::Vector> lambda_function(dof_handler,
//        lambda);
//    Vector<double> lambda_p1(dim);
//
//    // Create file and filename for output
//    std::string filename = ("LineInContact-"
//        + Utilities::int_to_string(cycle, 2) + "-"
//        + Utilities::int_to_string(
//            triangulation.locally_owned_subdomain(), 4) + ".dat");
//    std::fstream file;
...
//        stress_tensor = stress_strain_tensor * strain_tensor;
//
//        lambda_function.vector_value(p1, lambda_p1);
//
//        // output order: position along line + displacement + lambda + stress tensor
//        // position ux  uy  uz lambdax lambday lambdaz  sxx  syy  szz  sxy  sxz  syz
//        file << p / 100.0 << " " << solution_p1(0) << " "
//            << solution_p1(1) << " " << solution_p1(2) << " "
//            << lambda_p1(0) << " " << lambda_p1(1) << " "
//            << lambda_p1(2) << stress_tensor[0][0] << " "
//            << stress_tensor[1][1] << " " << stress_tensor[2][2]
//            << " " << stress_tensor[0][1] << " "
//            << stress_tensor[1][2] << " " << stress_tensor[1][2]
//            << std::endl;
//      }
